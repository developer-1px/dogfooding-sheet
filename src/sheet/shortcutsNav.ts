...
export function handleNavigation(e: KeyboardEvent, mod: boolean, a: NavArgs): boolean {
  const { focusId, cells, rowCount, colLetters, setSelectedIds, setFocusId } = a
  ...
  if (e.key === 'Tab') {
    const t = tabTarget(focusId, e.shiftKey, colLetters); if (t) { setFocusId(t); e.preventDefault() }; return true
  }
  ...
  if (e.key === 'Home' || e.key === 'End') {
    const t = homeEndTarget(focusId, rowCount, e.key, mod, colLetters); if (t) { setFocusId(t); e.preventDefault() }; return true
  }
