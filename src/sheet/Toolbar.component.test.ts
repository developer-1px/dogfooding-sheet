...
describe('Toolbar component', () => {
  ...
  it('renders toolbar command buttons as non-submit controls', () => {
    ...
    const autoSum = document.querySelector<HTMLButtonElement>('button[aria-label="B2에 자동 합계"]')
    ...
    expect(autoSum?.getAttribute('aria-label')).toBe('B2에 자동 합계')
    expect(autoSum?.getAttribute('title')).toBe('B2에 자동 합계 (위쪽 연속 숫자 합)')
    ...
    expect(centerAlign?.getAttribute('title')).toBe('B2 가운데 정렬 꺼짐')
  ...
  it('labels the active alignment button state', () => {
    ...
    const leftAlign = document.querySelector<HTMLButtonElement>('button[aria-label="B2 왼쪽 정렬 꺼짐"]')
    const centerAlign = document.querySelector<HTMLButtonElement>('button[aria-label="B2 가운데 정렬 켜짐"]')
    const rightAlign = document.querySelector<HTMLButtonElement>('button[aria-label="B2 오른쪽 정렬 꺼짐"]')
    ...
    expect(centerAlign?.getAttribute('title')).toBe('B2 가운데 정렬 켜짐')
    expect(rightAlign?.getAttribute('aria-pressed')).toBe('false')
    expect(rightAlign?.getAttribute('title')).toBe('B2 오른쪽 정렬 꺼짐')
  })
  ...
  it('keeps direct toolbar command activation keys inside the toolbar controls', async () => {
    ...
    const directButtons = [
      ...
      document.querySelector<HTMLButtonElement>('button[aria-label="선택 셀 2개 왼쪽 정렬 꺼짐"]'),
  ...
  it('disables formatting controls without target cells', () => {
    ...
    expect(leftAlign?.hasAttribute('aria-pressed')).toBe(false)
    expect(centerAlign?.textContent).toBe('⇔')
    expect(centerAlign?.disabled).toBe(true)
    expect(centerAlign?.getAttribute('title')).toBe('가운데 정렬할 셀 없음')
    expect(centerAlign?.hasAttribute('aria-pressed')).toBe(false)
    expect(rightAlign?.textContent).toBe('⇥')
    ...
    expect(plainFormat?.hasAttribute('aria-pressed')).toBe(false)
    expect(plainFormat?.getAttribute('title')).toBe('일반 적용할 셀 없음')
    expect(plainFormat?.hasAttribute('aria-keyshortcuts')).toBe(false)
    expect(percentFormat?.textContent).toBe('%')
    expect(percentFormat?.disabled).toBe(true)
    expect(percentFormat?.hasAttribute('aria-pressed')).toBe(false)
    expect(percentFormat?.getAttribute('title')).toBe('백분율 적용할 셀 없음')
    expect(percentFormat?.hasAttribute('aria-keyshortcuts')).toBe(false)
    expect(eurFormat?.textContent).toBe('€')
    expect(eurFormat?.disabled).toBe(true)
    expect(eurFormat?.hasAttribute('aria-pressed')).toBe(false)
