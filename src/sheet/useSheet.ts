...
export function useSheet(opts: { openGoto?: () => void; openNote?: () => void } = {}) {
  ...
  const tabFns = tabActions(tabs.state, tabs.setState, sheet.cells, (c) => ops.reset({ cells: c, notes: sheet.notes, styles: sheet.styles, formats: sheet.formats, validation: sheet.validation, condFormat: sheet.condFormat, freeze: sheet.freeze, hidden: sheet.hidden }))
