...
export function Grid({ ctx }: { ctx: SheetCtx }) {
  ...
  const cellMenu = useCellMenu({ sheet, colLetters: ctx.colLetters, hiddenRows: hiddenRowsManual, hiddenCols, filterCol: ctx.filter?.col ?? null, clearFilter: ctx.clearFilter, setFocusId, writeCell, insertRow, deleteRow, insertCol, deleteCol, appendRows: ctx.appendRows, appendCols: ctx.appendCols, sortByCol, noteOf, setNote, hideCol, hideRow, showRow: ctx.showRow, showCol: ctx.showCol, editNote: ctx.editNote, insertLink: ctx.insertLink, promptRowHeight: ctx.promptRowHeight, promptColWidth: ctx.promptColWidth, promptFilter: ctx.promptFilter, setFreezeRows: ctx.setFreezeRows, setFreezeCols: ctx.setFreezeCols, freeze, mergeSelection: ctx.mergeSelection })
