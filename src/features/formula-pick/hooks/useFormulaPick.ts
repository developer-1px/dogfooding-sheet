...
import { cycleTrailingFormulaRef, idsForFormulaPick, moveCellIdByDelta, refForFormulaPick, replaceTrailingFormulaRef } from '@spredsheet/grid'
...
export function useFormulaPick({ edit, rowCount, colLetters, setSelectedIds, setSelectAnchor }: Args) {
  ...
  const moveFormulaPick = useCallback((delta: { dRow: number; dCol: number }, extend = false) => {
    ...
    const next = moveCellIdByDelta(base, delta.dRow, delta.dCol, { rowCount, colLetters })
