...
import { MAX_CSV_EXPORT_LENGTH } from '../../../shared/lib/csv'
import { MAX_JSON_STRINGIFY_LENGTH } from '../../../shared/lib/jsonStringify'
import { initialSheet, type Sheet } from '../../../entities/Sheet/schema'
...
describe('overflowMenuActions', () => {
  it('builds stateful menu labels and validates menu ids', () => {
    ...
    expect(overflowMenuItems({ showFormulas: true, showGridlines: false })
      .find((item) => item.id === 'show-formulas')).toMatchObject({ kind: 'menuitemcheckbox', checked: true })
    expect(overflowMenuItems({ showFormulas: true, showGridlines: false })
    ...
    expect(overflowMenuItems({ showFormulas: false, showGridlines: false, canInsertLink: true })
      .find((item) => item.id === 'link')?.disabled).toBe(false)
    expect(overflowMenuItems({ showFormulas: false, showGridlines: false, canClearValues: false, canClearFormats: false })
      .filter((item) => item.id === 'clear-values' || item.id === 'clear-formats')
      .map((item) => item.disabled)).toEqual([true, true])
    expect(overflowMenuItems({ showFormulas: false, showGridlines: false, canClearValues: true, canClearFormats: true })
      .filter((item) => item.id === 'clear-values' || item.id === 'clear-formats')
      .map((item) => item.disabled)).toEqual([false, false])
