...
import { MAX_SHEET_TABS, blankBundle } from '../../../entities/Sheet/schema'
import { keyDown, setInputValue } from '../../../shared/testing/test-utils'
...
const overlaysCss = () => readFileSync('src/app/overlays.css', 'utf8')
...
describe('Tabs component', () => {
  ...
  it('keeps tab utility controls from shrinking inside tabs', () => {
    ...
    expect(duplicateRule).toContain('display: inline-flex;')
    expect(duplicateRule).toContain('align-items: center;')
    expect(duplicateRule).toContain('justify-content: center;')
    expect(duplicateRule).toContain('flex: 0 0 auto;')
    expect(duplicateRule).toContain('width: var(--sheet-size-control-md, 18px);')
    expect(duplicateRule).toContain('height: var(--sheet-size-control-md, 18px);')
    expect(duplicateRule).toContain('padding: 0;')
    expect(duplicateRule).toContain('line-height: var(--sheet-line-height-compact, 1);')
    expect(duplicateRule).not.toContain('padding: 0 var(--sheet-space-1, 4px);')
  ...
  it('keeps the tab rename input contained inside the tab width', () => {
    ...
    const renameFocusRule = css.match(/\.tab-rename:focus\s*\{[^}]+\}/)?.[0] ?? ''
    ...
    expect(renameFocusRule).toContain('outline: var(--sheet-focus-ring, 2px solid #1a73e8);')
    expect(renameFocusRule).toContain('outline-offset: var(--sheet-focus-offset, 2px);')
    expect(renameFocusRule).not.toContain('outline: none;')
  })
