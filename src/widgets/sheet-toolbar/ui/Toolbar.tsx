...
import type { Format, FormatLookup } from '../../../features/formatting/hooks/useFormats'
import type { SheetMutations } from '../../../features/structure/model/sheetMutations'
import type { FreezeState, FreezeActions } from '../../../features/visibility/hooks/useFreeze'
import type { CellStyle, StyleLookup } from '../../../features/formatting/hooks/useStyles'
import type { Ask } from '../../../shared/ports/dialog'
import type { CondActions } from '../../../features/formatting/hooks/useCondFormat'
...
import { CondFmtButtons } from '../../../features/formatting/ui/CondFmtButtons'
import { FormatButtons } from '../../../features/formatting/ui/FormatButtons'
import { StyleToggleButtons } from '../../../features/formatting/ui/StyleToggleButtons'
...
export function Toolbar({ display, writeCell, writeCells, writeCellRange, focusKey, selectedIds, setFormat, formatOf, insertRow, deleteRow, insertCol, deleteCol, appendRows, appendCols, sortByCol, updateStyle, styleOf, freeze, toggleFreezeRows, toggleFreezeCols, filter, applyFilter, clearFilter, hasHidden, showAll, setListRule, setCheckboxRule, clearRule, openHelp, insertLink, addCondRule, clearCondRules, condRuleCount, sheet, previewSheetReplacement, applySheetReplacement, clearCellValues, ask, confirm, undo, redo, canUndo, canRedo, showFormulas, toggleShowFormulas, showGridlines, toggleShowGridlines, clearAllFormats, mergeSelection, rowCount, colCount }: Props) {
  ...
  const clearFormatKeyShortcuts = hasCellTarget ? 'Control+\\ Meta+\\' : undefined
  ...
  const activeFreezeRowsPressed = canToggleFreezeRows ? freezeRowsPressed : undefined
  const activeFreezeColsPressed = canToggleFreezeCols ? freezeColsPressed : undefined
  const canOpenFilterPrompt = !!focus && rowCount > 1
  const activeFilterPressed = canOpenFilterPrompt || filter ? !!filter : undefined
  ...
  const mergeKeyShortcuts = canMerge ? 'Alt+Shift+M' : undefined
  ...
  return (
    <>
      ...
      <label className="color-pick" title={bgColorLabel} aria-disabled={colorPickerDisabled}>🎨<input type="color" value={bgColorValue} title={bgColorLabel} aria-label={bgColorLabel} disabled={!hasCellTarget} onKeyDown={stopToolbarActivationKeyDown} onChange={(e) => setBg(e.target.value)} /></label>
      <label className="color-pick" title={fgColorLabel} aria-disabled={colorPickerDisabled}>A<input type="color" value={fgColorValue} title={fgColorLabel} aria-label={fgColorLabel} disabled={!hasCellTarget} onKeyDown={stopToolbarActivationKeyDown} onChange={(e) => setFg(e.target.value)} /></label>
      ...
      <button {...toolbarCommandButtonProps} onClick={toggleFreezeRows} disabled={!canToggleFreezeRows} title={freezeRowsLabel} aria-label={freezeRowsLabel} aria-pressed={activeFreezeRowsPressed} style={freezeRowsPressed ? activeToolbarStateStyle : undefined}>📌행{freeze.rows > 1 ? `×${freeze.rows}` : ''}</button><button {...toolbarCommandButtonProps} onClick={toggleFreezeCols} disabled={!canToggleFreezeCols} title={freezeColsLabel} aria-label={freezeColsLabel} aria-pressed={activeFreezeColsPressed} style={freezeColsPressed ? activeToolbarStateStyle : undefined}>📌열{freeze.cols > 1 ? `×${freeze.cols}` : ''}</button>
