...
import { keyDown } from '../../../shared/testing/test-utils'
...
describe('OverflowMenu component', () => {
  ...
  it('labels the compact trigger and keeps menu-button relationships', () => {
    ...
    expect(items.find((item) => item.textContent === '수식 표시 (Ctrl/⌘+`)')?.getAttribute('title')).toBe('수식 표시 (Ctrl/⌘+`) 꺼짐')
  ...
  it('exposes checkbox state for view toggles', () => {
    ...
    expect(formula?.getAttribute('title')).toBe('수식 표시 (Ctrl/⌘+`) 켜짐')
    expect(formula?.getAttribute('aria-label')).toBe('수식 표시 (Ctrl/⌘+`) 켜짐')
    ...
    expect(gridlines?.getAttribute('title')).toBe('격자선 표시 꺼짐')
    expect(gridlines?.getAttribute('aria-label')).toBe('격자선 표시 꺼짐')
  ...
  it('disables destructive clear items when the sheet has nothing to clear', () => {
    ...
    expect(clearValues?.getAttribute('title')).toBe('지울 셀 값 없음')
    expect(clearValues?.getAttribute('aria-label')).toBe('지울 셀 값 없음')
