...
import { setupReactDOM } from '../../../shared/testing/test-utils'
import { Cell } from './Cell'

describe('Cell component', () => {
  const dom = setupReactDOM()
  const appCss = () => readFileSync('src/app/App.css', 'utf8')
  const gridCss = () => readFileSync('src/widgets/sheet-grid/grid.css', 'utf8')
  ...
  it('keeps URL link keyboard events inside the link control', () => {
    ...
    expect(link?.getAttribute('aria-keyshortcuts')).toBe('Enter')
  ...
  it('keeps wrapped textarea editor keyboard events inside the editor control', () => {
    ...
    expect(textarea?.getAttribute('title')).toBe('A1 편집 (Enter=저장 / Esc=취소)')
    expect(textarea?.getAttribute('aria-keyshortcuts')).toBe('Enter Escape')
