...
import { cellId, cellKey, parseCellId, type FillCellRange, type Sheet, type SheetOps, type WriteCell, type WriteCellRange, type WriteMany, type Display } from '../../../entities/Sheet/schema'
import { copyOrCut, pasteAt } from '../../../features/clipboard/model/clipboardActions'
import type { ClipboardTextBridge } from '../../../features/clipboard/model/clipboardActions'
import { fillDown, fillRight } from '../../../features/fill/model/fillDown'
...
export function useGlobalShortcuts(get: () => GlobalShortcutCtx) {
  ...
  useShortcut('mod+d', () => { const c = get(); if (c.selectedIds.length > 1) fillDown(c.selectedIds, c.fillCellRange) })
  useShortcut('mod+r', () => { const c = get(); if (c.selectedIds.length > 1) fillRight(c.selectedIds, c.fillCellRange) })
}
