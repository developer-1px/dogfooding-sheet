...
import { largeSmall, rank, sumproduct, sample, weightAvg, arrayToText, firstLast, maxMinBy, lenStat, rangeHash, strStat, countNumeric, mostCommon } from './rangeOps'
...
export function dispatch(fn: string, rawArgs: string, c: Ctx): string {
  ...
  if (F === 'MOSTCOMMON') return smartReturn(mostCommon(splitArgs(rawArgs)[0], c.cells, c.evalRaw))
