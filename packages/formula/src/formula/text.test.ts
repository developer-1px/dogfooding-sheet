...
describe('text functions', () => {
  ...
  it('basic text functions reject missing required inputs', () => {
    expect(evaluateCell({}, '=LEN()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=UPPER()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=LOWER()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=PROPER()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=LEFT()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=RIGHT()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=MID("abc", 2)')).toBe('#VALUE!')
    expect(evaluateCell({}, '=TRIM()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=NORMALIZE()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=CLEAN()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=T()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=LPAD()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=RPAD()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=REVERSE()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=REPT()')).toBe('#VALUE!')
    expect(evaluateCell({}, '=STARTSWITH("hello")')).toBe('#VALUE!')
    expect(evaluateCell({}, '=FIND("h")')).toBe('#VALUE!')
    expect(evaluateCell({}, '=SEARCH("h")')).toBe('#VALUE!')
    expect(evaluateCell({}, '=REPLACE("abc", 1)')).toBe('#VALUE!')
  })
  it('LEFT / RIGHT / MID', () => {
