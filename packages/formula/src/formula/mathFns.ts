...
export function dispatchMath(F: string, argsT: string[], argsN: number[]): string | null {
  ...
  if (F === 'MEDIAN3') { const s = [...argsN].sort((a, b) => a - b); return String(s[1]) }
  if (F === 'INVLERP') { const [v, a, b] = argsN; return b === a ? '#DIV/0!' : String((v - a) / (b - a)) }
  ...
  if (F === 'RANDNORM') {
    ...
    return String(mu + sd * Math.sqrt(-2 * Math.log(u)) * Math.cos(2 * Math.PI * v))
  }
  ...
  if (F === 'SECH') return String(1 / Math.cosh(argsN[0]))
  if (F === 'CSCH') return String(1 / Math.sinh(argsN[0]))
  if (F === 'COTH') return String(1 / Math.tanh(argsN[0]))
