...
describe('math functions', () => {
  ...
  it('TRUNC / SIGN / PI / EVEN / ODD', () => {
    ...
    expect(evaluateCell({}, '=ROUND(1.5, 0)')).toBe('2')
    expect(evaluateCell({}, '=ROUND(-1.5, 0)')).toBe('-2')
    expect(evaluateCell({}, '=ROUND(-1.25, 1)')).toBe('-1.3')
    expect(evaluateCell({}, '=ROUND(125, -1)')).toBe('130')
  ...
  it('MROUND / QUOTIENT / SQRTPI', () => {
    ...
    expect(evaluateCell({}, '=MROUND(2.5, 1)')).toBe('3')
    expect(evaluateCell({}, '=MROUND(-2.5, -1)')).toBe('-3')
    expect(evaluateCell({}, '=MROUND(-2.5, 1)')).toBe('#NUM!')
    expect(evaluateCell({}, '=MROUND(2.5, -1)')).toBe('#NUM!')
    expect(evaluateCell({}, '=MOD(10, 3)')).toBe('1')
    expect(evaluateCell({}, '=MOD(-3, 2)')).toBe('1')
    expect(evaluateCell({}, '=MOD(3, -2)')).toBe('-1')
    expect(evaluateCell({}, '=MOD(3, 0)')).toBe('#DIV/0!')
