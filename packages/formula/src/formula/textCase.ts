...
export function dispatchTextCase(F: string, argsT: string[]): string | null {
  if (F === 'UPPER') return argsT[0] === undefined ? wrap('#VALUE!') : wrap(argsT[0].toUpperCase())
  if (F === 'LOWER') return argsT[0] === undefined ? wrap('#VALUE!') : wrap(argsT[0].toLowerCase())
  if (F === 'PROPER') return argsT[0] === undefined ? wrap('#VALUE!') : wrap(argsT[0].toLowerCase().replace(/(^|\s)(\p{L})/gu, (_m, sp, ch) => sp + ch.toUpperCase()))
