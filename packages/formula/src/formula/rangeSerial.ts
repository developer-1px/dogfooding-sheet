...
export const rangeJSON = (r: string, c: Cells, e: EvalCell) =>
  stringifyFormulaArray(vals(r, c, e))

/** RANGECSV(range) — values comma-separated, double-quote escaped if needed. */
export const rangeCsv = (r: string, c: Cells, e: EvalCell) => {
...
export function rangeUnique(r: string, c: Cells, e: EvalCell): string {
...
export function rangeSort(r: string, c: Cells, e: EvalCell): string {
