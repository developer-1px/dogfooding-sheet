import { ABS_A1_RE, colIndex, columnLabel } from '../coordinates/a1'
...
export function shiftFormulaRows(raw: string, fromRow: number, delta: 1 | -1, rowCount = Infinity): string {
  ...
  return '=' + raw.slice(1).replace(ABS_A1_RE, (match, absCol: string, col: string, absRow: string, rowText: string) => {
    ...
    return `${absCol}${col}${absRow}${nextRow1}`
  })
}
...
export function shiftFormulaCols(raw: string, fromCol: number, delta: 1 | -1): string {
  ...
  return '=' + raw.slice(1).replace(ABS_A1_RE, (match, absCol: string, col: string, absRow: string, rowText: string) => {
    ...
    return nextCol ? `${absCol}${nextCol}${absRow}${rowText}` : '#REF!'
  })
}
