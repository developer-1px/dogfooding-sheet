import { pad2 } from '../compute/numeric'
...
export function tryNamedCycle(source: string[], target: number): string[] | null {
  for (const cycle of CYCLES) {
    const idx = cycle.indexOf(source[0])
    if (idx >= 0 && source.every((s, i) => s === cycle[(idx + i) % cycle.length])) {
      const out = [...source]
      for (let i = source.length; i < target; i++) out.push(cycle[(idx + i) % cycle.length])
      return out
    }
  }
  return null
